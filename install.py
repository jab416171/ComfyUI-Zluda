...
def main():
    ...
    conda_env_dir = os.path.join(install_dir, ".venv")
    ...
    run_with_progress(f'conda create -y -p {conda_env_dir} python=3.11', "Creating Conda environment")

    # Step 3: Activate the Conda environment
    activate_command = f'conda activate {conda_env_dir} && '
    ...
    subprocess.run(activate_command + f'{os.path.join(conda_env_dir, "Scripts", "pip")} install -r requirements.txt', shell=True, check=True)
    ...
    torch_lib_dir = os.path.join(conda_env_dir, 'Lib', 'site-packages', 'torch', 'lib')
